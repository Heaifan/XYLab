// UA1/STAT-1 · Canvas 调度层：Scatter 额外消费 MonitorSnapshot.statistics，禁止 UI 重算轴统计。
...
interface Props {
  def: ExperimentDefinition | null; snap: MonitorSnapshot; view: ViewState; targets: string[]; ctx: VizCtx;
  lockTime: number | null; onLock: (t: number | null) => void; onToggleSelect: (t: string) => void;
  setView: (v: ViewState) => void;
}

export function VizHost(p: Props) {
  const chk = checkVizById(p.view.viz, p.ctx), viz = p.view.viz;
  const legend = <div className="chart-head">
    <div className="legend">
      {p.targets.map((tg, i) => <button key={tg} className={`legend-item legend-btn s-${(i % 4) + 1}`} title="点击移出图表（Inspector 同步）" onClick={() => p.onToggleSelect(tg)}>
        <i />{labelOf(p.def, tg)}
      </button>)}
    </div>
  ...
  if (p.targets.length === 0 && viz !== 'timeline' && viz !== 'etrack') return <p className="muted">未选择指标——在「监控值」点击行加入图表。</p>;
  if (!chk.ok) return <>{showSeriesLegend && legend}<p className="muted viz-reason">当前不可用：{chk.reason}</p></>;
  ...
  if (viz === 'line' || viz === 'area' || viz === 'step') {
    ...
    chart = <ScatterChart series={p.snap.series} stats={p.snap.statistics} def={p.def} targets={p.targets} xT={xT} yT={yT}
