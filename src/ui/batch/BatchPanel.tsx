// Batch Experiment BATCH-2 · JSON 自动方案 + 可靠复制/CRUD/运行反馈；可视化仍消费 XYUI-8。
...
export function BatchPanel({ definition }: Props) {
  ...
  const activeTarget = targets.includes(chartTarget) ? chartTarget : (targets[0] ?? ''), baseline = results.find((r) => r.scenarioId === baselineId);
  ...
  return <section className="panel batch-panel"><div className="batch-title"><h2>方案实验</h2><span className="muted">BATCH-2 · XYUI</span></div>
    {definition.batch && <BatchPreview definition={definition} scenarios={scenarios} />}
    <div className="row batch-actions"><label>每方案模拟 <input type="number" min={1} value={count} disabled={running || !!definition.batch?.tickLimit} onChange={(e) => setCount(Math.max(1, Math.floor(Number(e.target.value) || 1)))} /></label>{!definition.batch && <button disabled={running} onClick={add}>+ 添加方案</button>}<button className="primary" disabled={running || !scenarios.length} onClick={runAll}>{running ? `运行中 ${results.length}/${scenarios.length}` : `运行全部 ${scenarios.length} 个方案`}</button><ActionFeedback feedback={feedback} /></div>
    ...
    {!!results.length && <div className="batch-results"><div className="batch-result-head"><b>方案对比</b><div className="row"><button onClick={() => copy(batchJson)}>复制对比 JSON</button><button onClick={() => { downloadJson(`${definition.experiment.id}-batch-result.json`, batchJson); note('结果 JSON 已下载'); }}>下载 JSON</button></div></div>
