// Batch Experiment V1-F1 · 比较绑定、方案快照查看、可分享结果 JSON。
...
export function BatchPanel({ definition }: Props) {
  ...
  async function runAll() {
    if (!definition || scenarios.length === 0) return; setRunning(true); setResults([]); setDetailId('');
    try { await runBatch(definition, scenarios, count, (result) => setResults((old) => [...old, result])); } finally { setRunning(false); }
  }
  function inspect(id: string) {
    if (!definition) return; const s = scenarios.find((item) => item.id === id); if (!s) return;
    setDetailId(id); setView({ ...VIEW_INIT, selected: initSelection(withInitialValues(definition, s.overrides)) }); setLockTime(null);
  }
  ...
  const batchJson = batchResultExport(definition, scenarios, results, baselineId, count, activeTarget);
  const scenarioJson = detail && scenario ? scenarioResultExport(definition, scenario, detail, count) : null;
  return <section className="panel batch-panel">
    ...
    {!!results.length && <div className="batch-results">
      <div className="batch-result-head"><b>方案对比</b><div className="row"><button onClick={() => copy(batchJson)}>复制对比 JSON</button><button onClick={() => downloadJson(`${definition.experiment.id}-batch-result.json`, batchJson)}>下载 JSON</button></div></div>
      ...
      {!!rows.length && <div className="hbars">{rows.map((r, i) => <div className="hbar-row" key={r.id}><span className="hbar-label">{r.name}</span><span className="hbar-track"><span className={`hbar-fill s-${i % 4 + 1}`} style={{ width: `${Math.abs(r.value) / max * 100}%` }} /></span><b className="hbar-value">{formatNumber(r.value)}</b></div>)}</div>}
      <div className="batch-table-wrap"><table className="vtable batch-table"><thead><tr><th>方案</th><th>参数</th><th>状态</th><th>{definition.variables[activeTarget]?.label ?? activeTarget}</th><th>查看</th></tr></thead><tbody>{results.map((r) => {
        const s = scenarios.find((item) => item.id === r.scenarioId), v = r.values[activeTarget], bv = baseline?.values[activeTarget], delta = numeric(v) && numeric(bv) ? v - bv : null;
        const params = s ? Object.entries(s.overrides).map(([k, x]) => `${definition.variables[k]?.label ?? k}=${String(x)}${definition.variables[k]?.unit ?? ''}`).join(' · ') : '—';
        return <tr key={r.scenarioId}><td>{r.name}</td><td>{params || '基准参数'}</td><td>{r.error ? '失败' : r.status === 'completed' ? '完成' : r.status}</td><td><b>{numeric(v) ? formatNumber(v) : String(v ?? '—')}</b>{delta !== null && r.scenarioId !== baselineId && <small className={delta >= 0 ? 'delta-up' : 'delta-down'}>{delta >= 0 ? '+' : ''}{formatNumber(delta)}</small>}</td><td><button onClick={() => inspect(r.scenarioId)}>查看结果</button></td></tr>;
      })}</tbody></table></div>
      {detail && detailDef && scenarioJson && <div className="batch-detail"><div className="batch-result-head"><b>{detail.name} · 结果可视化</b><div className="row"><button onClick={() => copy(scenarioJson)}>复制方案 JSON</button><button onClick={() => downloadJson(`${definition.experiment.id}-${detail.name}-result.json`, scenarioJson)}>下载 JSON</button><button onClick={() => setDetailId('')}>关闭</button></div></div><VisualizationPanel definition={detailDef} snap={detail.snapshot} lockTime={lockTime} onLock={setLockTime} view={view} setView={setView} resolved={resolved} breakpoint={breakpoint} toast={toast} onToggleSelect={(t) => { const n = selectToggle(view, detailDef, t); setView(n.view); setToast(n.toast); }} onClear={() => setView(viewClearSelect(view))} /></div>}
    </div>}
  </section>;
}
