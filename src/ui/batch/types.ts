// Batch Experiment V1 · 方案覆盖、结果快照与可分享 JSON 合同。
...
export function batchResultExport(
  ...
  return {
    ...
    scenarios: results.map((result) => {
      ...
      return {
        id: result.scenarioId, name: result.name, overrides: scenario?.overrides ?? {},
...
export function scenarioResultExport(
  ...
  const targets = Object.keys(result.snapshot.series);
  return {
    schemaVersion: 'xylab-scenario-result@0.1', generatedAt: new Date().toISOString(),
    experiment: definition.experiment, random: definition.random ?? null,
    scenario: { id: scenario.id, name: scenario.name, overrides: scenario.overrides },
