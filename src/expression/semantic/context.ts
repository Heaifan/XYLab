...
// 克制原则：仅暴露协议明确允许的常量与函数；random 由运行时随机源注入。

import type { ExperimentDefinition } from '../../protocol/types';
import type { FunctionSignature, SemanticContext, SymbolType, SemanticType } from './types';

export const BUILTINS: ReadonlyMap<string, SemanticType> = new Map([
  ['dt', 'number'],
  ['PI', 'number'],
]);
...
export const FUNCTIONS: ReadonlyMap<string, FunctionSignature> = new Map([
  ...
  ['sin', { minArgs: 1, maxArgs: 1, paramType: 'number', returnType: 'number' }],
  ['cos', { minArgs: 1, maxArgs: 1, paramType: 'number', returnType: 'number' }],
  ['random', { minArgs: 0, maxArgs: 0, paramType: 'number', returnType: 'number' }],
]);
