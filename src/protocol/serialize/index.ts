// BATCH-3/MSV-1 · 内部可信 Definition → 可再次由 Loader 读取的外部实验 JSON。
...
function externalBatch(definition: ExperimentDefinition): Record<string, unknown> | undefined {
  ...
  const out: Record<string, unknown> = { mode: definition.batch.mode ?? 'matrix', dimensions: definition.batch.dimensions.map((dimension) => ({
