// BATCH-2/MSV-1 · Batch 引用、值域、Seed Sweep 与方案数量语义校验。
...
export function checkBatch(raw: RawExperiment, errors: LoadError[]): void {
  if (!raw.batch) return;
  const dimensions = raw.batch.dimensions as RawDimension[], seen = new Set<string>(); let total = 1;
  dimensions.forEach((dim, i) => {
    ...
    if (dim.values) {
      ...
      total *= dim.values.length; return;
    }
