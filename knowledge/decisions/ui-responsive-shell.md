...
**三模式断点冻结（断点值不变，组合位置 FE-A-R2 修订、F2 再修订）**：

```text
Wide    ≥1024  → 左(实验+参数) | 中(动作+运行+可视化) | 右(Inspector+监控值行, 280~420) | 底(日志)
Medium  ≥640   → 辅助栏(实验+参数,可折叠) | 主区(动作+运行+可视化+监控值行+Inspector) | 底(日志)
